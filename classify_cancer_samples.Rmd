...
Can we detect iNKTs?
```{r eval=F}
model <- scGate::gating_model(level=1, positive=T, name="CD8T", signature = c("CD8A","CD8B"))
model <- scGate::gating_model(model, level=1, negative=T, name="iNKT", signature = c("ZBTB16"))

gated.list <- lapply(gated.list,
       FUN = function(x) {
          tmp <- subset(x, subset=is.pure=="Pure")
          scGate(tmp, model=model, ncores=4, neg.thr = 0.01)
       })
```
...
```{r eval=F, fig.height=15}
pll <- lapply(names(projected),
       FUN = function(x) {
          DefaultAssay(projected[[x]]) <- "RNA"
          FeaturePlot(projected[[x]], features=c("Zbtb16")) + NoLegend()
       })
...
pl <- FeaturePlot(projected.merged.all, combine = F, max.cutoff = 'q98',
            features=c("Cd8a","Cd4","Tcf7","Ccr7","Cd69","Gzmk","Gzmb","Havcr2","Lag3","Entpd1",
                       "Cxcr6","Mki67","Top2a","Anxa1","Gzmm","Zbtb16","Hivep3","Ifit3","Hsp90aa1","Sell")) 
#pl <- lapply(pl, function(x) {
  # x + scale_color_viridis(option="D", direction=1)
#})
wrap_plots(pl)

#table(x$patient, x$cluster)
```
...
pl <- FeaturePlot(projected.merged.Temex, combine = F, max.cutoff = 'q98',
            features=c("Havcr2","Cxcl13","Lag3","Gzmk","Anxa1","Mki67")) 
      #      features=c("Psma1","Psmd8","Psma5","Anxa1","Lag3","Mki67")) 
pl <- lapply(pl, function(x) {
...
ggsave("plots/markers.integrated.UMAP.png", height=5, width=10)
```

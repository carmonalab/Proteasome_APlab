...
DimPlot(seurat.cd8, reduction = "umap", group.by = "cellSubType") + theme(aspect.ratio=1)
ggsave("plots/bassez.subtypes.UMAP.png", height=5, width=8)

DimPlot(seurat.cd8, reduction = "umap", group.by = "patient_id")  + theme(aspect.ratio=1)
DimPlot(seurat.cd8, reduction = "umap", group.by = "expansion")  + theme(aspect.ratio=1)
ggsave("plots/bassez.exp.UMAP.png", height=5, width=8)
...
pll <- FeaturePlot(seurat.cd8, reduction = "umap", features = names(signatures), ncol = 3, combine=F, raster = T, pt.size = 2)

pll <- lapply(pll, function(x) {
   x + scale_color_viridis(option="D", direction=-1) + theme(aspect.ratio=1)
})
wrap_plots(pll, ncol=3)   

ggsave("plots/bassez.signatures.UMAP.png", width=12, height=10)
    
```
```{r fig.height=5, fig.width=4}
Idents(seurat.cd8) <- "cellSubType"
VlnPlot(seurat.cd8, features=names(signatures),  pt.size = 0, stack=T, flip=T, fill.by="ident") + NoLegend()
ggsave("plots/bassez.signatures.vln.png", height=10, width=5)
```
...
data <- seurat.cd8@meta.data
ggplot(data , aes(x=PROTEASOME, y=EXHAUSTION)) +
  geom_point() + geom_smooth(method="auto", se=TRUE) + facet_grid(.~BC_type) + theme_light()

ggsave("plots/bassez.correlations.png", height=3, width=9)

data.s <- SplitObject(seurat.cd8, split.by = "BC_type")  

ct <- lapply (names(data.s), function(x) {
  cor.test(data.s[[x]]$PROTEASOME, data.s[[x]]$EXHAUSTION)
})
names(ct) <- names(data.s)
ct
```
...
Plot differential scores post- vs. pre-antiPD1
```{r}
...
ggsave("plots/bassez.pre.vs.post.therapy.scores.png", height=5, width=7)
```

Bin on Protesome-low and Proteasome_high
```{r}
sub <- c("CD8_EX","CD8_EX_Proliferating")
min_cells <- 20

hist(seurat.cd8$PROTEASOME, breaks=30)
prot.thr <- median(seurat.cd8$PROTEASOME)
...
pre.pp <- list()
df <- list()
post.pp[["high"]] <- subset(post, PROTEASOME>=prot.thr)
post.pp[["low"]] <- subset(post, PROTEASOME<prot.thr)
pre.pp[["high"]] <- subset(pre, PROTEASOME>=prot.thr)
pre.pp[["low"]] <- subset(pre, PROTEASOME<prot.thr)

for (p in c("low","high")) {
  differentials <- lapply(pass, function(x) {
    sub.post <- subset(post.pp[[p]], subset=patient_id == x)
    sub.pre <- subset(pre.pp[[p]], subset=patient_id == x)
  ...
  df[[p]] <- as.data.frame(t(as.data.frame(differentials)))
  colnames(df[[p]]) <- c("Proteasome","Exhaustion","Proliferation")
  df[[p]]$Prot_status <- p
}
df.byp <- rbind(df$low, df$high)

```


Plot differential scores post- vs. pre-antiPD1, binned by proteas. score
```{r}
melted <- reshape2::melt(df.byp[,c("Proteasome","Prot_status")], value.name = "Score_diff")
melted <- melted[,c(1,3)]

a <- ggplot(melted, aes(x=Prot_status, y=Score_diff)) +
  geom_boxplot() + 
  geom_jitter(position=position_jitter(0.2)) + 
  theme_light() + ggtitle("Proteasome score")

melted <- reshape2::melt(df.byp[,c("Exhaustion","Prot_status")], value.name = "Score_diff")
melted <- melted[,c(1,3)]

b <- ggplot(melted, aes(x=Prot_status, y=Score_diff)) +
  geom_boxplot() + 
  geom_jitter(position=position_jitter(0.2)) + 
  theme_light() + ggtitle("Exhaustion score")

melted <- reshape2::melt(df.byp[,c("Proliferation","Prot_status")], value.name = "Score_diff")
melted <- melted[,c(1,3)]

c <- ggplot(melted, aes(x=Prot_status, y=Score_diff)) +
  geom_boxplot() + 
  geom_jitter(position=position_jitter(0.2)) + 
  theme_light() + ggtitle("Proliferation score")
...
ggsave("plots/bassez.pre.vs.post.therapy.byprot_status.png", height=5, width=7)
```
...
```{r}
VlnPlot(seurat.cd8,features="PROTEASOME", split.by = "timepoint", group.by = "BCtype")
VlnPlot(seurat.cd8,features="PSMB5", split.by = "timepoint", group.by = "cellSubType")

VlnPlot(seurat.cd8 %>% subset(cellSubType == "CD8_EX"),features="PROTEASOME", 
        split.by = "timepoint", group.by = "BC_type", pt.size = 0)
ggsave("plots/vlnplot.Tex.by.cancertype.prot.png", height=4, width=5)

VlnPlot(seurat.cd8 %>% subset(cellSubType == "CD8_EX"),features="EXHAUSTION", 
        split.by = "timepoint", group.by = "BC_type", pt.size = 0)
ggsave("plots/vlnplot.Tex.by.cancertype.exh.png", height=4, width=5)
```
```{r}
VlnPlot(seurat.cd8,features="PROTEASOME", split.by = "expansion", group.by = "cellSubType")
VlnPlot(seurat.cd8,features="PSMB5", split.by = "expansion", group.by = "cellSubType")
```

...
VlnPlot(data_seurat,features = myFeaturesUcell, pt.size = 0, stack = T, flip = T, cols = cols.functional.cluster, fill.by	
="ident")
...
```{r}
#data_seurat@meta.data$T_EX_sub_T_EM_UCell <- data_seurat@meta.data$T_EX_UCell-data_seurat@meta.data$T_EM_UCell #double signature for higher discrimination
...
```{r}
ggplot(ww1,aes(x=feature,y=auc)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
...
```{r}
data_seurat@meta.data$EXHAUSTION_UCell_HIGH <- ifelse(data_seurat@meta.data$EXHAUSTION_UCell>0.15,"EXHAUSTION_UCell_HIGH","EXHAUSTION_UCell_LOW")
#data_seurat@meta.data$EXHAUSTION_UCell_HIGH <- ifelse(data_seurat@meta.data$EXHAUSTION_UCell>0.1,"EXHAUSTION_UCell_HIGH",ifelse(data_seurat@meta.data$EXHAUSTION_UCell==0,"EXHAUSTION_UCell_LOW",NA))
...
```
Find  PROTEASOMA sig genes correlation with exhaustion
